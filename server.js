...
function analyze(history, diceHistory) {
  ...
  /* ===== 5. CẦU ĐẶC BIỆT (Pattern) ===== */
  const pattern6 = history.slice(-6).join("-");
  const pattern5 = history.slice(-5).join("-");
  if (/^(Tài-Tài-Xỉu-Tài-Tài|Xỉu-Xỉu-Tài-Xỉu-Xỉu)$/.test(pattern6)) {
    return { prediction: last, confidence: 75 };
  }
  if (/^(Tài-Tài-Tài-Xỉu-Xỉu-Tài|Xỉu-Xỉu-Xỉu-Tài-Tài-Xỉu)$/.test(pattern6)) {
    return { prediction: last, confidence: 70 };
  }
  if (/^(Tài-Xỉu-Xỉu-Tài-Tài-Tài|Xỉu-Tài-Tài-Xỉu-Xỉu-Xỉu)$/.test(pattern6)) {
    return { prediction: last, confidence: 70 };
  }
  if (/^(Tài-Tài-Tài-Tài-Xỉu-Xỉu-Tài-Tài|Xỉu-Xỉu-Xỉu-Xỉu-Tài-Tài-Xỉu-Xỉu)$/.test(pattern6)) {
    return { prediction: last, confidence: 70 };
  }
  if (/^(Tài-Xỉu-Xỉu-Tài-Tài|Xỉu-Tài-Tài-Xỉu-Xỉu)$/.test(pattern5)) {
    return { prediction: last, confidence: 70 };
  }
    ...
    // Rule: 8-9-10 -> 13+ -> 11 => về Xỉu
    if (diceHistory.length >= 4) {
      const d1 = diceHistory.at(-4); // 8
      const d2 = diceHistory.at(-3); // 9
      const d3 = diceHistory.at(-2); // 10
      const d4 = diceHistory.at(-1); // 13 trở lên hoặc 11
      if (d1 === 8 && d2 === 9 && d3 === 10 && d4 >= 13) {
        return { prediction: "Xỉu", confidence: 85 };
      }
      if (d1 >= 13 && d2 === 11 && d3 <= 7) {
        return { prediction: "Xỉu", confidence: 80 };
    ...
    // Rule: Xỉu tăng dần từ thấp (<=6) -> 7-8 -> 9-10 => dự đoán Tài
    if (prev2Dice <= 6 && (prevDice === 7 || prevDice === 8) && (lastDice === 9 || lastDice === 10)) {
      return { prediction: "Tài", confidence: 75 };
    }

      if (prev2Dice === 12 && prevDice === 11) {
      return { prediction: "Xỉu", confidence: 80 };
    }

      if ((prevDice === 11 || prevDice === 12) && lastDice === 16) {
      return { prediction: "Xỉu", confidence: 85 };
    }

      if (
      history.length >= 2 &&
      history.at(-2) === "Xỉu" && (diceHistory.at(-2) === 7 || diceHistory.at(-2) === 8) &&
      history.at(-1) === "Xỉu" && diceHistory.at(-1) === 10
    ) {
      return { prediction: "Tài", confidence: 80 };
    }
